...
export default function Calendar() {
  ...
  const outgoingWeeks = useCalendar(
    outgoingDate?.getFullYear() ?? currentDate.getFullYear(),
    (outgoingDate?.getMonth() ?? currentDate.getMonth()) + 1
  );

  const incomingStyle = useAnimatedStyle(() => {
    return {
      transform: [{ translateX: (1 - animValue.value) * direction * width }],
    };
  });

  const outgoingStyle = useAnimatedStyle(() => {
    return {
      transform: [{ translateX: -animValue.value * direction * width }],
    };
  });
  ...
  return (
    <View style={styles.container}>
      ...
      <GestureDetector gesture={gesture}>
        <View style={{ height: 6 * 50, overflow: "hidden" }}>
          {outgoingDate && (
            <Animated.View style={[{ position: "absolute", width: "100%" }, outgoingStyle]}>
              <CalendarGrid
                weeks={outgoingWeeks}
                year={outgoingDate.getFullYear()}
                month={outgoingDate.getMonth() + 1}
                selected={null}
                setSelected={() => {}}
          ...
          <Animated.View style={[{ position: "absolute", width: "100%" }, incomingStyle]}>
            <CalendarGrid
              weeks={currentWeeks}
